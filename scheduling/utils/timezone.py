...
def convert_to_user_timezone(dt, user_tz_str):
    '''
    Convert a UTC datetime to the user's local timezone.
    Args: - dt: datetime object in UTC
        - user_tz_str: string representing the user's timezone, e.g. 'Europe/London
...
def convert_to_utc(dt, user_tz_str):
    '''
...
def format_datetime_for_user(dt, user_tz_str, fmt='%A, %d %B %Y at %H:%M'):
    '''
    Format a datetime object for display in the user's local timezone.
    '''
    local_dt = convert_to_user_timezone(dt, user_tz_str)
